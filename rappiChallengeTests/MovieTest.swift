...
class MovieTest: XCTestCase {
    ...
    func test_Image_backdropPath_Exist() {
        XCTAssertNotNil(movie.backdropPath)
    }
    func test_Image_posterPath_Exist() {
        XCTAssertNotNil(movie.posterPath)
    }
    func test_test_Image_posterURL_success(){
        XCTAssertEqual(movie.posterURL, URL(string: "https://image.tmdb.org/t/p/w500\(movie.posterPath ?? "")"))
    }
    func test_test_Image_backdropPath_success(){
        XCTAssertEqual(movie.backdropURL, URL(string: "https://image.tmdb.org/t/p/w500\(movie.backdropPath ?? "")"))
    }
}
